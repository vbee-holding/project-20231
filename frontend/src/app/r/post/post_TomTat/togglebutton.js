...
const ToggleButton = (props) => {
  ...
  useEffect(() => {
    ...
    if (router.pathname === '/r/post/post_ChiTiet') {
      setIsToggled(false);
    }
  }, [router.pathname]);
  ...
  const handleClick = () => {
    ...
    router.push('/r/post/post_ChiTiet');
  };
    ...
    <div className="relative flex flex-col items-center justify-center overflow-hidden">
      <div className="flex">
        <label className="inline-flex relative items-center mr-5 cursor-pointer">
          ...
          <span className="ml-2 text-sm font-medium text-gray-900">{isToggled ? 'Tóm Tắt' : 'Chi Tiết'}</span>
        </label>
      </div>
    </div>
  );
